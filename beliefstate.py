...
from models.contextset import *

def create_dummy_cell():
    return DictCell({'bit': BoolCell(),
                     'num': IntervalCell(0, 2000),
                     'sub': {'part': BoolCell(),
                             'size': IntervalCell(0, 2000)},
                     'size': IntervalCell(0, 2000)})

def create_dummy_contextset(size=3):
    import random
    cs = ContextSet()
    for i in range(5):
        ds = create_dummy_cell()
        ds.num = i
        ds.bit = False
        ds.sub.part = True
        ds.sub.size = 5 
        ds.size = 5
        cs.cells.append(ds)
    cs.name = "Dummy Context Set" + str(random.random())
    return cs

def test_update_contextset():
    cs = create_dummy_contextset(5)
    b = BeliefState()
    b.contextset =  cs
    assert b.number_of_targets() == 5
    b.merge(['target', 'bit'], T)
    assert b.number_of_targets() == 0

def time_beliefstate_copy():
    from exterior.timer import Timer
    cs = create_dummy_contextset(5)
    b = BeliefState()
    b.contextset =  cs
    with Timer() as t:
        for _ in xrange(500):
            d2 = b.copy()
            d2.merge(['target','sub','part'], False)
            assert d2.number_of_targets() != b.number_of_targets()
    print t.secs, "seconds"

if __name__ == '__main__':
    test_update_contextset()
    cs = create_dummy_contextset(5)
    b = BeliefState()
    b.contextset =  cs
    print hash(b)
    sizes = list(b.get_paths_for_attribute('size'))
    b1 = b.copy()
    b1.merge(sizes[0], 5)
    b2 = b.copy()
    b2.merge(sizes[1], 5)
    ...
    b4 = b.copy()
    b4.merge(sizes[0], 10)
    assert hash(b1) != hash(b2), "Nested attributes have same hash"
    assert hash(b2) != hash(b3), "Nested attributes have same hash"
    assert hash(b3) != hash(b4), "Nested attributes have same hash"
    assert hash(b1) != hash(b4), "Nested attributes have same hash"
    assert hash(b2) != hash(b4), "Nested attributes have same hash"
    assert hash(b3) != hash(b1), "Nested attributes have same hash"
